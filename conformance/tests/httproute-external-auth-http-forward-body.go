...
// HTTPRouteExternalAuthHTTPForwardBody verifies the forwardBody.maxSize behaviour of
// the HTTP ExternalAuth filter. The manifest defines two rules on the same HTTPRoute:
//
//   - Rule 1 (maxSize=100): bodies within the limit are forwarded to the auth server;
//     bodies exceeding it must be rejected with 4xx. The auth server echoes the received
//     body size as X-Auth-Received-Body-Size, which the gateway forwards to the upstream
//     via allowedResponseHeaders, letting the backend confirm the size.
//
//   - Rule 2 (maxSize=0): the body must not be forwarded to the auth server regardless
//     of its size. The gateway must not reject requests solely because they carry a body.
...
var HTTPRouteExternalAuthHTTPForwardBody = confsuite.ConformanceTest{
	...
	Description: "An HTTPRoute ExternalAuth HTTP filter honours forwardBody.maxSize: bodies within the limit are forwarded and confirmed via X-Auth-Received-Body-Size; bodies over the limit are rejected with 4xx; maxSize=0 withholds the body entirely",
