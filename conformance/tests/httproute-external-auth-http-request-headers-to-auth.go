...
// HTTPRouteExternalAuthHTTPRequestHeadersToAuth verifies that headers listed in
// the filter's http.allowedHeaders are forwarded to the HTTP auth server.
//
// The auth server echoes the value of X-Test-Header back as X-Auth-Received-Test-Header.
// The filter's http.allowedResponseHeaders list includes that header so the gateway
// adds it to the upstream request, where the backend echoes it back to the client.
