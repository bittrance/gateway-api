...
// HTTPRouteExternalAuthBackendUnavailable verifies that the ExternalAuth filter is
// fail-closed: when the auth Service exists but has no healthy endpoints (its pod
// selector matches no running pods), requests must be denied rather than being
// forwarded to the backend without authentication.
//
// This distinguishes a runtime connectivity failure from a missing Service reference:
//   - The ExternalAuth backendRef resolves correctly (Service exists with the declared
//     port), so ResolvedRefs is True.
//   - At request time the auth server is unreachable, so the gateway must not
//     silently pass the request through.
