...
// HTTPRouteExternalAuthGRPCRequestHeadersToAuth verifies that headers listed in
// the filter's grpc.allowedHeaders are forwarded to the gRPC auth server.
//
// The auth server echoes the value of X-Test-Header back as X-Auth-Received-Test-Header
// in its OkHttpResponse, which the gateway adds to the upstream request. The
// backend echoes all request headers, so the test can confirm the round-trip.
