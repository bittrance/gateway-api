...
// HTTPRouteExternalAuthPerRouteScope verifies that the ExternalAuth filter is scoped
// to the individual HTTPRoute rule that declares it and is not applied globally to all
// routes on the same listener.
//
// Background: some data-plane implementations (notably Envoy) configure ext_authz at the
// HTTP Connection Manager (HCM) level, which — if not careful — can cause auth to be
// enforced on every route in the listener, even routes that have no ExternalAuth filter.
//
// Test design:
//
//   - Route "with-auth" matches prefix /http and has an ExternalAuth HTTP filter.
//     The request GET /http/allowed passes through the auth server, which allows it → 200.
//
//   - Route "without-auth" matches the more-specific prefix /http/denied (no filter).
//     Per HTTPRoute path-precedence rules, this route wins for GET /http/denied, bypassing
//     the auth server entirely → 200 from the backend.
//
//     If auth were incorrectly applied at the listener/HCM level, the auth server would
//     receive /http/denied and return 403, causing the second assertion to fail.
