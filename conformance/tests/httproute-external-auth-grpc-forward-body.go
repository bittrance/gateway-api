...
// HTTPRouteExternalAuthGRPCForwardBody verifies the forwardBody.maxSize behaviour of
// the gRPC ExternalAuth filter. ForwardBody is a top-level field of HTTPExternalAuthFilter
// and applies to both the HTTP and GRPC protocols. When using gRPC, the body is carried in
// CheckRequest.attributes.request.http.body (or raw_body). The manifest defines two rules:
//
//   - Rule 1 (maxSize=100): bodies within the limit are placed in the CheckRequest and the
//     request succeeds; bodies exceeding it must be rejected with 4xx. The auth server echoes
//     the received body size as X-Auth-Received-Body-Size in OkHttpResponse.Headers; the
//     gateway adds it to the upstream request so the backend can confirm the size.
//
//   - Rule 2 (maxSize=0): the body and raw_body fields of the CheckRequest must be empty.
//     The gateway must not reject requests solely because they carry a body.
//     X-Auth-Received-Body-Size: 0 confirms the body was withheld — observable via the
//     upstream request on the allowed path and via the client 403 response on the denied path.
//
...
var HTTPRouteExternalAuthGRPCForwardBody = confsuite.ConformanceTest{
	...
	Description: "An HTTPRoute ExternalAuth gRPC filter honours forwardBody.maxSize: bodies within the limit are forwarded and confirmed via X-Auth-Received-Body-Size; bodies over the limit are rejected with 4xx; maxSize=0 withholds the body entirely",
